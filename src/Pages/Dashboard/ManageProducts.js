...
const ManageProducts = () => {
    ...
    const getUsers = async () => {
        const { data } = await axiosPrivate.get('https://parts-bd-inc-server.vercel.app/parts')
        return data
    }
    ...
    const deleteProductHandle = async _id => {
        const { data } = await axiosPrivate.delete(`https://parts-bd-inc-server.vercel.app/parts/${_id}`)
        if (data.success) {
            refetch()
